...
function renderTherapyCards(){
  ...
  grid.innerHTML=THERAPIES.map((th,i)=>`
    <div class="tc" tabindex="0" aria-label="${th.name}">
      <div class="tc-media">
        <img src="${th.imgs[0]}" alt="${th.name}" loading="lazy">
        <button class="tc-see-more" onclick="openTherapyModal(${i});event.stopPropagation()"><i class="fas fa-eye"></i> See More</button>
      </div>
      <div class="tc-body">
        <h3>${th.name}</h3>
        <p>${th.short}</p>
        <div class="tc-meta">
          ...
          <span class="tc-dur"><i class="fas fa-clock"></i> ${th.dur}</span>
        </div>
        <div class="tc-btns">
          <button class="btn-gold btn-sm" onclick="navTo('contact')">Book Now</button>
          <button class="btn-outline btn-sm" onclick="openTherapyModal(${i})">See More</button>
        </div>
      </div>
    </div>
  `).join('');
...
function openTherapyModal(idx){
  ...
  content.innerHTML=`
    <div class="tm-name">${th.name}</div>
    <div class="tm-meta">
      <span class="tm-price">${th.price}</span>
      <span class="tm-dur"><i class="fas fa-clock"></i> ${th.dur}</span>
    </div>
    ...
    <p class="tm-desc">${th.long}</p>
    <div class="tm-benefits">
      <h4>Physical Benefits</h4>
      <ul>${th.phys.map(b=>`<li>${b}</li>`).join('')}</ul>
      <h4>Mental Benefits</h4>
      <ul>${th.ment.map(b=>`<li>${b}</li>`).join('')}</ul>
      <h4>Who is it for?</h4>
      <p>${th.forWhom}</p>
    </div>
    <button class="btn-gold btn-full" style="margin-top:20px" onclick="navTo('contact');closeTherapyModal()">Book Now — ${th.price}</button>
  `;
